...
cat(sprintf("[INFO] Input list has %d unique genes listed\n", length(genes)))  #  [oai_citation:0‡rdocumentation.org](https://www.rdocumentation.org/packages/base/versions/3.6.2/topics/cat?utm_source=chatgpt.com)

# ── 2. Connect to the correct Ensembl dataset ───────────────────────────────────
# Run to see all datasets:
#listDatasets(ensembl)

#    Tip: choose the nearest mirror
ensembl <- biomaRt::useEnsembl(
  biomart = "genes",
  dataset = "hsapiens_gene_ensembl",   # change to e.g. mmusculus_gene_ensembl for mouse
  mirror  = "useast"                   #  "asia", or drop this line for default
)
